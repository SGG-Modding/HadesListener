...
local pcall, loadfile, select, rawipairs, rawpairs, yield = pcall, loadfile, select, rawipairs or ipairs, rawpairs or pairs, coroutine.yield
...
function StyxScribe.AddHook( callback, prefix, source )
	if source ~= nil then
		...
		elseif type( prefix ) == "table" then
			prefix = table.concat( prefix, delim ) .. delim
		end
	end
	local base = hooks[ prefix ] or { }
	table.insert( base, callback )
	hooks[ prefix ] = base
	send( prefixStyxScribe, 
		"Adding hook on \"" .. prefix .. 
		"\" with " .. tostring( callback ) ..
...
StyxScribe.Internal = ModUtil.UpValues( function( )
	return debugMode, pollDelay, pollPeriod, notify, proxyFile, showDebugPrint, showDebugAssert,
		...
		startswith, vararg, hooks, poke, poll, handle, waitArgs, delim
end )